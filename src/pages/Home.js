...
export default function Home() {
  ...
  const {expiryDates, underlyingValue, strikePrices, timestamp, filtered } = options;
  console.log({expiryDates, underlyingValue, strikePrices, timestamp, filtered });
  ...
  const expiryDateOptions = useMemo(() => {
    ...
    const expiry_date_options = expiryDates.map((item) => ({lable: item, value: item}));
  ...
  const strikePriceOptions = useMemo(() => {
    ...
    const strike_price_options = strikePrices.map((item) => ({lable: item, value: item}));
  ...
  const {filteredOptions, loICall, loIPut, lVolCall, lVolPut} = useMemo(() => {
    ...
    const filtered_data = filtered.data.filter((item) => item.expiryDate === selectedExpiryDate?.value);
    // open interess
